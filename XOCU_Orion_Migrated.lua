...
-- Keeps the existing feature callbacks while moving the entire UI to Orion.
local Library = {
    Flags = {},
    Items = {},
}

local function _keycode(v)
    if typeof(v) == "EnumItem" then return v end
    if type(v) ~= "string" or v == "" or v == "None" then
        return Enum.KeyCode.Unknown
    end
    return Enum.KeyCode[v] or Enum.KeyCode.Unknown
end

local function _control(control, flag, defaultValue)
    local wrapper = {
        _control = control,
        Flag = flag,
        Value = defaultValue,
    ...
    function wrapper:Set(value)
        ...
        if self._control and self._control.Set then
            pcall(function() self._control:Set(value) end)
        end
    end
    ...
    function wrapper:Refresh(list, keepSelection)
        if self._control and self._control.Refresh then
            pcall(function() self._control:Refresh(list, keepSelection) end)
        end
    end

    function wrapper:SetItems(list, keepSelection)
        self:Refresh(list, keepSelection)
    end
...
function Library:CreateWindow(opts)
    ...
    local win = OrionLib:MakeWindow({
        Name = opts.Title or "Orion",
        HidePremium = false,
        SaveConfig = opts.AutoSave ~= false,
        ConfigFolder = opts.ConfigFolder or "OrionConfig",
        IntroEnabled = false,
        IntroText = opts.Title or "Orion",
        Icon = opts.CustomIcon and ("rbxassetid://" .. tostring(opts.CustomIcon)) or nil,
    })

    local Window = {}

    function Window:CreateTab(name, _enabled, icon)
        local tab = win:MakeTab({
            Name = name,
            Icon = (type(icon) == "string" and icon:match("^rbxassetid://"))
                and icon
                or ("rbxassetid://" .. tostring(icon or "4483345998")),
            PremiumOnly = false,
        })

        local Tab = {}

        function Tab:CreateBlock(blockOpts)
            blockOpts = blockOpts or {}
            local section = tab:AddSection({
                Name = blockOpts.Name or "Section",
            })

            local Block = {}

            function Block:CreateToggle(o)
                o = o or {}
                local flag = o.Flag or o.Name
                local current = o.Default == true
                local c = tab:AddToggle({
                    Name = o.Name or flag,
                    Default = current,
                    Save = o.Save ~= false,
                    Flag = flag,
                    Callback = function(v)
                        current = v
                        if Library.Flags[flag] then Library.Flags[flag].Value = v end
                        if o.Callback then o.Callback(v) end
                    end,
                })
                local w = _control(c, flag, current)
                Library.Flags[flag] = w
                Library.Items[flag] = w
                return w
            end

            function Block:CreateSlider(o)
                o = o or {}
                local flag = o.Flag or o.Name
                local current = o.Default or o.Min or 0
                local c = tab:AddSlider({
                    Name = o.Name or flag,
                    Min = o.Min or 0,
                    Max = o.Max or 100,
                    Default = current,
                    Color = Color3.fromRGB(255,255,255),
                    Increment = o.Increment or 1,
                    ValueName = o.ValueName or "",
                    Save = o.Save ~= false,
                    Flag = flag,
                    Callback = function(v)
                        current = v
            ...
            function Block:CreateDropdown(o)
                o = o or {}
                local flag = o.Flag or o.Name
                local items = o.Items or o.List or o.Options or {}
                local current = o.Default
                local c = tab:AddDropdown({
                    Name = o.Name or flag,
                    Default = current,
                    Options = items,
                    Save = o.Save ~= false,
            ...
            function Block:CreateInput(o)
                o = o or {}
                local flag = o.Flag or o.Name
                local c = tab:AddTextbox({
                    Name = o.Name or flag,
                    Default = o.Default or "",
                    TextDisappear = o.TextDisappear == true,
                    Callback = function(v)
                        if o.Callback then o.Callback(v) end
                    end,
                })
                local w = _control(c, flag, o.Default or "")
                Library.Flags[flag] = w
                Library.Items[flag] = w
                return w
            end

            function Block:CreateKeybind(o)
                o = o or {}
                local flag = o.Flag or o.Name
                return tab:AddBind({
                    Name = o.Name or flag,
                    Default = _keycode(o.Default),
                    Hold = o.Hold == true,
                    Callback = o.Callback or function() end,
                })
            end

            function Block:CreateColorpicker(o)
                o = o or {}
                local flag = o.Flag or o.Name
                ...
                local w = _control(c, flag, o.Default or Color3.new(1,1,1))
                Library.Flags[flag] = w
                Library.Items[flag] = w
                return w
            end

            function Block:CreateLabel(text)
                return tab:AddLabel(text or "")
            end

            function Block:CreateParagraph(title, content)
                return tab:AddParagraph(title or "", content or "")
            end
...
function Library:Notify(opts, content, duration)
    if type(opts) == "table" then
        OrionLib:MakeNotification({
            Name = opts.Title or opts.Name or "Notification",
            Content = opts.Content or opts.Description or "",
            Image = opts.Image or "rbxassetid://4483345998",
            Time = opts.Duration or opts.Time or 5,
        })
    else
        OrionLib:MakeNotification({
            Name = tostring(opts or "Notification"),
...
function AutoExplosionLoop()
    ...
    Players.PlayerRemoving:Connect(function(plr)
        ...
        if plr.Name == _expTarget then
            ...
            _expTarget = nil
            pcall(function()
                Toggles.AutoExplosionToggle:Set(false)
            end)
        end
        updateDropdown()
    end)
    ...
    KeybindsGroup:CreateKeybind({
        ...
        Callback = function()
            if Toggles.LoopTpToggle then
                Toggles.LoopTpToggle:Set(not Toggles.LoopTpToggle:GetValue())
            end
        end
    })
...
-- Orion initialization (required by Orion Library)
OrionLib:Init()
