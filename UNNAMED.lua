...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/sladkoeshkaogg-svg/UNNAMED/refs/heads/main/UNNAMED%20FAKELIBRORY.lua"))()
...
function handleMessage(sender, text)
    ...
    elseif cmd == '.credits' then
        for _, target in ipairs(targets)do
            if target == LocalPlayer then
                sendToChat('CREDITS: Made by UNNAMED and 9rr')
            end
        end
    end
end
...
function sendHubLoadedMessage()
	local message = " Owner Version | UNNAMED loaded. "
	local sent = false
	pcall(function()
            ...
            _G.AutoLeaveCon = game:GetService("ReplicatedStorage").GameCorrectionEvents.GameCorrectionsNotify.OnClientEvent:Connect(function(r)
                if r == "Flying" then
                    ...
                    if #warnTimestamps >= 3 then
                        game:GetService("Players").LocalPlayer:Kick("UNNAMED Safety: Disconnected to prevent ban.")
                    end
                end
            end)
        end
...
do
    ...
    local modeOptions = {
        "Loop Kick Blob",
        "UNNAMED (grab + blob)",
        "UNNAMED spam blob loop",
        "UNNAMED Kill Blob [Fast]"
    }
    ...
    BlobGroup:CreateToggle({
        ...
        Callback = function(State)
            ...
            elseif selectedBlobMode == "UNNAMED (grab + blob)" then
                blobTask = task.spawn(function()
                    local RS = game:GetService("ReplicatedStorage")
                    local RunService = game:GetService("RunService")
            ...
            elseif selectedBlobMode == "UNNAMED spam blob loop" then
                blobTask = task.spawn(function()
                    while blobActive do
                        local target = selectedKickPlayer
                        local char = LocalPlayer.Character
            ...
            elseif selectedBlobMode == "UNNAMED Kill Blob [Fast]" then
                blobTask = task.spawn(function()
                    while blobActive do
                        pcall(function()
                            local char = LocalPlayer.Character
...
do
    ...
    -- ==========================================================
    -- UNNAMED UI INTEGRATION (Trimmed)
    -- ==========================================================
...
function notifyKick(displayName, username)
	Library:Notify({ Title = "UNNAMED ", Content = displayName .. " (" .. username .. ") has been kicked", Duration = 6,
	 })
end
...
do
    ...
    FX_defs["Orbit Rings"] = function()
        ...
        for i = 1, count do
            ...
            ring.Name="UNNAMEDFXPart"; ring.Size=Vector3.new(7,.18,.18)
    ...
    FX_defs["Bubble Shield"] = function()
        ...
        sphere.Name="UNNAMEDFXPart"; sphere.Size=Vector3.new(8,8,8)
    ...
    FX_defs["Star Burst"] = function()
        ...
        for i=1,count do
            ...
            s.Name="UNNAMEDFXPart"; s.Size=Vector3.new(.4,.4,.4)
    ...
    FX_defs["Ice Shards"] = function()
        ...
        for i=1,6 do
            local s=Instance.new("Part")
            s.Name="UNNAMEDFXPart"; s.Size=Vector3.new(.3,1.4+math.random()*.8,.3)
        ...
        task.spawn(function()
            while alive and Visuals.FX.ActiveEnabled do
                ...
                if hrp then
                    ...
                    m.Name="UNNAMEDMeteor"; m.Size=Vector3.new(1,1,1)
...
print("UNNAMED Loaded Xocu and 9rr so goated")
